...
// Phase 4B §7 — canonical domain rule: a follow-up is a 'message' step like
// any other; WAIT defines the delay before it, and its position in the
// sequence is what makes it a "relance." 'follow_up' is kept in the type
// (and in the DB check constraint) ONLY for backward-compatible reads of
// rows created before this rule — no new campaign_steps row should ever be
// created with step_type='follow_up' again (see createCampaign/saveSteps
// callers). The Campaign Engine already treats it as a generic, non-'end'
// step_type, so old rows keep advancing correctly; they are simply never
// specifically executed until a WhatsApp/Email executor is registered.
...
// Campaign Strategy lives inside campaigns.settings (the same jsonb column
// description/stopOnReply already use) — a persisted, per-campaign object,
// not React state, but not a new table either: it's a single 1:1 structured
// extra on a campaign, exactly what settings already exists for.
export async function getCampaignStrategy(context:WorkspaceContext,campaignId:string): Promise<CampaignStrategy|null> { const r=await database.query<{settings:{strategy?:CampaignStrategy}}>(`select settings from campaigns where workspace_id=$1 and id=$2`,[context.workspaceId,campaignId]); return r.rows[0]?.settings?.strategy??null; }
export async function saveCampaignStrategy(context:WorkspaceContext,campaignId:string,strategy:CampaignStrategy): Promise<CampaignRecord|null> { const client=await database.connect(); try { const existing=await getCampaignRow(client,context.workspaceId,campaignId); if(!existing)return null; await client.query(`update campaigns set settings=$1,updated_at=now() where workspace_id=$2 and id=$3`,[{...existing.settings,strategy},context.workspaceId,campaignId]); await activity(client,context,campaignId,"campaign.strategy_updated"); return record(client,context.workspaceId,(await getCampaignRow(client,context.workspaceId,campaignId))!); } finally { client.release(); } }
