...
export function CampaignsClient() {
  ...
  const refresh=async()=>{const [campaignResponse,contactResponse,whatsappRelationsResponse]=await Promise.all([fetch("/api/campaigns"),fetch("/api/contacts"),fetch("/api/campaigns/whatsapp-relations")]);if(!campaignResponse.ok||!contactResponse.ok){setError("Impossible de charger les campagnes.");return;}const campaignData=await campaignResponse.json() as {campaigns:ApiCampaign[]};const contactData=await contactResponse.json() as {contacts:Contact[]};setCampaigns(campaignData.campaigns.map(mapCampaign));setContacts(contactData.contacts);if(whatsappRelationsResponse.ok){const relationsData=await whatsappRelationsResponse.json() as {relations:WhatsAppRelation[]};setWhatsappRelations(relationsData.relations);}};
  ...
  const resetWizard = () => { setWizardOpen(false); setStep(0); setName(""); setObjective("Prospecter"); setChannel("linkedin"); setSelectedContacts([]); setWaitDays(3); setSearch(""); setInitialMessage(PROSPECTING_MESSAGE_DEFAULT); setFollowUpMessage("Bonjour {first_name}, je me permets de revenir vers vous."); };
...
// needs to know that.
function WhatsAppParticipantsPanel({ campaignId }: { campaignId: string }) {
