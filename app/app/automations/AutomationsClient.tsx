  ...
  return <div className="automations-page automations-page--v2">
    ...
    <Dialog description="Cette règle est enregistrée dans votre espace de travail Talvia." onClose={() => setFormOpen(false)} open={formOpen} title={editing ? "Modifier l’automatisation" : "Nouvelle automatisation"}>
      <form className="workspace-form automation-builder" onSubmit={(event) => void save(event)}>
        ...
        {form.action === "create_reply_draft" ? <fieldset><legend>Réponse automatique en environnement sandbox</legend><label><span>Mode de réponse</span><select onChange={(event) => setForm({ ...form, replyMode: event.target.value as Form["replyMode"] })} value={form.replyMode}><option value="draft">Brouillon uniquement</option><option value="auto">Simulation d’envoi automatique</option></select></label><label><span>Délai avant action (minutes)</span><input min="0" onChange={(event) => setForm({ ...form, delayMinutes: event.target.value })} type="number" value={form.delayMinutes} /></label><p className="automation-simulation">Simulation uniquement — aucun message réel ne sera envoyé.</p></fieldset> : null}
