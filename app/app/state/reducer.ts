...
export function sandboxReducer(
  ...
  switch (action.type) {
    ...
    case "CREATE_CONTACT":
      return runAutomations({ ...state, contacts: [...state.contacts, action.contact] }, { type: "contact_added", contactId: action.contact.id, channel: action.contact.channel });
    case "CREATE_OPPORTUNITY":
      return runAutomations({
        ...state,
        opportunities: [...state.opportunities, action.opportunity],
      }, { type: "opportunity_created", opportunityId: action.opportunity.id, contactId: action.opportunity.contactId, channel: action.opportunity.sourceChannel });
    case "UPDATE_OPPORTUNITY":
      { const previous = state.opportunities.find((item) => item.id === action.opportunity.id); const next = { ...state, opportunities: state.opportunities.map((item) => item.id === action.opportunity.id ? action.opportunity : item) }; return action.opportunity.stage === "proposal" && previous?.stage !== "proposal" ? runAutomations(next, { type: "opportunity_proposal", opportunityId: action.opportunity.id, contactId: action.opportunity.contactId, channel: action.opportunity.sourceChannel }) : next; }
    ...
    case "CREATE_MESSAGE":
      return action.message.direction === "inbound" ? runAutomations({ ...state, messages: [...(state.messages ?? []), action.message] }, { type: "message_received", contactId: action.message.contactId, channel: action.message.channel }) : { ...state, messages: [...(state.messages ?? []), action.message] };
