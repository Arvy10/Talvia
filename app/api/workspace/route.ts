...
export async function GET() {
  try {
    ...
    const result = await database.query(
      `select w.id,w.name,w.default_locale,w.default_timezone,w.ai_instructions,u.first_name,u.last_name,u.email
...
export async function PATCH(request: Request) {
  try {
    const context = await getCurrentWorkspace();
    const input = await request.json() as { name?: string; firstName?: string; lastName?: string; locale?: string; timezone?: string; aiInstructions?: string };
    ...
    if (input.aiInstructions !== undefined) await database.query("update workspaces set ai_instructions=$1,updated_at=now() where id=$2", [input.aiInstructions.slice(0, 4000), context.workspaceId]);
